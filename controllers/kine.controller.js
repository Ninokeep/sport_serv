...
exports.updateEntrainementForUser = async(req,res) => {
  ...
  try{
    connection.execute(
      'UPDATE `session` INNER JOIN `session_meta` ON session.id = session_meta.id_session SET session_meta.meta_value  = ?, session.repetition_fait = ? , session.commentaire_kine = ?, session.fini = ? WHERE session.id_user = ? AND session.id = ? AND session.id_kine = ? ', [meta_value,repetition_fait,commentaire_kine,fini, id_user, id_session, id_kine],
        function (err, results, fields) {
          if(err){
            res.status(404).json({success:false,response:"problème de requête"})
          }
          else{
          res.status(200).json({ success: true, response: results });
