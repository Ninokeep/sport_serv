...
exports.create = async(req,res) => {
    ...
    if(!errors.isEmpty()){
       ...
       try{
        const rule = false
        const sportif = await User.create({nom,prenom,rule, email,sexe,password,age,numero_telephone,seance_restante,id_kine,pathologie,password: bcrypt.hashSync(password,10)});
        
        res.status(200).json({"success":true,"response":sportif})  
